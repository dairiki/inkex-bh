...
@pytest.mark.usefixtures("assert_quiet")
...
@pytest.mark.usefixtures("verbose_logging", "assert_no_stdout")
def test_run_command_verbose(capsys):
    run_command([sys.executable, "-c", "print('output')"])
    assert "output" in capsys.readouterr().err


@pytest.mark.usefixtures("assert_no_stdout")
def test_run_command_missing_ok(capsys):
    run_command(["this-does-not-exist-f38csed"], missing_ok=True)
    assert "Can not find executable" in capsys.readouterr().err
...
@pytest.mark.usefixtures("mock_export_png", "assert_quiet")
...
@pytest.mark.usefixtures("mock_export_png", "assert_no_stdout")
def test_recreate_stale_inset(svg_maker, run_effect, tmp_path, capsys):
    ...
    assert "missing_id" in capsys.readouterr().err
...
def test_recreate_inset_spurious_selections(svg_maker, run_effect, tmp_path, capsys):
    ...
    assert "Ignoring non-insets in selection" in capsys.readouterr().err


@pytest.mark.usefixtures("assert_no_stdout")
def test_create_inset_no_selection(svg_maker, run_effect, tmp_path, capsys):
    assert run_effect(svg_maker.as_file()) is None
    assert "No objects selected" in capsys.readouterr().err
...
def test_create_inset_multiple_selections(svg_maker, run_effect, tmp_path, capsys):
    ...
    assert "Select exactly one object" in capsys.readouterr().err
